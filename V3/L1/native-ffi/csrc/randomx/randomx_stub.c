 ...
 *
 *  The stub returns a deterministic Keccak-256 of (header ++ nonce) so
 *  testnet/CI can exercise the call path end-to-end.
 *
 *  PRODUCTION NOTE:
 *    Replace this file with the real RandomX implementation by linking against
 *    https://github.com/tevador/RandomX
 *    and wrapping randomx_calculate_hash() with this same ABI signature.
 *
 *  Functions exported (matching Rust FFI in native_ffi.rs):
 *    void randomx_zion_init(void)
...
/* ---- Minimal Keccak-256 (Ethereum-pad, no external deps) ---- */
...
static void keccakf(uint64_t s[25]) {
    ...
    for(r=0;r<24;r++){
        ...
        for(i=0;i<5;i++){t=bc[(i+4)%5]^ROTL64(bc[(i+1)%5],1);for(j=0;j<25;j+=5)s[j+i]^=t;}
        t=s[1];
        for(i=0;i<24;i++){j=PI[i];bc[0]=s[j];s[j]=ROTL64(t,RHO[i]);t=bc[0];}
...
static void keccak256(const uint8_t*in,size_t inlen,uint8_t*out){
    ...
    for(;inlen>=r;inlen-=r,in+=r){for(i=0;i<r/8;i++)s[i]^=((uint64_t*)in)[i];keccakf(s);}
    memcpy(tmp,in,inlen); tmp[inlen++]=0x01;
    memset(tmp+inlen,0,r-inlen); tmp[r-1]|=0x80;
    for(i=0;i<r/8;i++)s[i]^=((uint64_t*)tmp)[i]; keccakf(s);
    memcpy(out,s,32);
}

/* ---- Public API ---- */

EXPORT void randomx_zion_init(void) {
    /* no-op for stub */
}
